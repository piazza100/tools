...
export default function PublicPriceStatisticsPage({kind}:{kind:Kind}){
 ...
 const query=useMemo(()=>new URLSearchParams(window.location.search),[]),queryDate=(name:string,fallback:string)=>/^\d{4}-\d{2}-\d{2}$/.test(query.get(name)||'')?query.get(name)!:fallback
 const [from,setFrom]=useState(()=>queryDate('from',iso(monthAgo))),[to,setTo]=useState(()=>queryDate('to',iso(today))),[itemCode,setItemCode]=useState(()=>query.get('itemCode')||''),regionCode=query.get('regionCode')||'1101'
 ...
 useEffect(()=>{if(query.get('itemCode')&&items.some(item=>item.itemCode===itemCode)&&!result&&!loading)void search()},[items])
 ...
 const tableUrl=()=>{const params=new URLSearchParams({from,to});if(itemCode)params.set('itemCode',itemCode);if(regional)params.set('regionCode',regionCode);return `${regional?'/data/regional-prices':'/data/retail-price-history'}?${params}`}
 return <div className="app"><SiteHeader/><main className="lookup-page"><a className="price-back" href="/#data">← 생활 자료</a>
  ...
  <nav className="lookup-view-tabs" aria-label={`${title} 보기`}><a href={tableUrl()}>가격표</a><a className="active" href={`${regional?'/data/regional-prices/statistics':'/data/retail-price-history/statistics'}?${new URLSearchParams({from,to,itemCode,...(regional?{regionCode}:{})})}`}>통계 그래프</a></nav>
...
function PeriodChart({result}:{result:PublicPriceStatistics}){
 const points=result.points,width=900,height=300,padX=92,padY=30,values=points.flatMap(point=>[point.minPrice,point.averagePrice,point.maxPrice]),min=Math.min(...values),max=Math.max(...values),range=max-min||1,ticks=Array.from({length:5},(_,index)=>max-range*index/4)
 ...
 return <div className="statistics-chart"><div className="chart-legend"><b>세로축: 조사 가격(원)</b><span className="min">최저</span><span className="avg">평균</span><span className="max">최고</span></div><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${result.itemName} 날짜별 가격 추이, 세로축 원`}>{ticks.map(value=>{const tickY=y(value);return <g className="y-tick" key={value}><line x1={padX} y1={tickY} x2={width-padX} y2={tickY}/><text x={padX-10} y={tickY+4} textAnchor="end">{won(value)}</text></g>})}<line className="axis" x1={padX} y1={padY} x2={padX} y2={height-padY}/><line className="axis" x1={padX} y1={height-padY} x2={width-padX} y2={height-padY}/><polyline className="min-line" points={line('minPrice')}/><polyline className="avg-line" points={line('averagePrice')}/><polyline className="max-line" points={line('maxPrice')}/>{points.map((point,index)=><g key={point.label}><circle cx={x(index)} cy={y(point.averagePrice)} r="4"><title>{point.label} 평균 {won(point.averagePrice)}</title></circle>{(index%labelEvery===0||index===points.length-1)&&<text x={x(index)} y={height-8} textAnchor="middle">{point.label.slice(5)}</text>}</g>)}</svg></div>
}
