export type HousingPlanInput={
 ...
 firstInterimMonth:string;interimInterval:number;annualLoanRate:number;balanceRate:number;acquisitionTaxRate:number;incidentalCost:number;loanRounds?:number[]
...
export function housingPlan(input:HousingPlanInput):HousingPlanResult{
 ...
 const loanDraws=Array.from({length:interimCount},(_,i)=>({round:i+1,month:firstInterim+i*Math.max(1,input.interimInterval),amount:installment})).filter(x=>loanRoundSet.has(x.round))
 ...
 for(let i=0;i<interimCount;i++){
  const month=firstInterim+i*Math.max(1,input.interimInterval)
