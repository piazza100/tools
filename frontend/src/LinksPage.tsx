...
export default function LinksPage(){
 ...
 return <div className="app"><header className="site-header"><a className="brand" href="/"><i>W</i><span>WonderLife<small>Everyday answers, made simple.</small></span></a><nav><a href="/#tools">계산기</a><a href="/#data">생활 자료</a><a href="/links" aria-current="page">생활 사이트</a><a href="/guides">이용 가이드</a><a href="/#history">계산 이력</a></nav><div className="header-actions"><details ref={menuRef} className="mobile-menu"><summary>메뉴</summary><div onClick={closeMenu}><a href="/#tools">계산기</a><a href="/#data">생활 자료</a><a href="/links">생활 사이트</a><a href="/guides">이용 가이드</a><a href="/#history">전체 계산 이력</a></div></details></div></header>
 ...
 <section className="site-grid" aria-live="polite">{visible.map(site=><article key={site.name}><div className="site-card-heading"><small>{site.category}</small><a href={site.url} target="_blank" rel="noopener noreferrer" aria-label={`${site.name} 외부 사이트 방문`}><h2>{site.name}</h2><span aria-hidden="true">↗</span></a></div><p>{site.description}</p><div className="site-tags">{site.tags.map(tag=><span key={tag}>#{tag}</span>)}</div>{site.notice&&<p className="site-notice">{site.notice}</p>}</article>)}</section>{!visible.length&&<div className="empty-history">검색 조건에 맞는 사이트가 없습니다.</div>}
