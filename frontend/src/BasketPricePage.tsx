...
export default function BasketPricePage(){
 ...
 const items=useMemo(()=>((data?.items||[]).map(item=>({...item,quantity:selected[item.itemCode]??item.quantity})).filter(item=>item.quantity>0)),[data,selected])
 ...
 return <div className="price-app"><PriceHeader/><main className="price-page">
  ...
  <section className="price-hero"><div><p className="eyebrow">MY BASKET PRICE INDEX</p><h1>내 장바구니 물가는<br/><em>얼마나 달라졌을까요?</em></h1><p>매일 수집한 가격으로 내가 자주 사는 품목의 변화를 한눈에 확인합니다.</p></div><div className="price-total"><small>오늘의 장바구니</small><strong>{won.format(summary.total)}</strong><span className={(summary.monthRate||0)>0?'up':'down'}>한 달 전보다 {rate(summary.monthRate)}</span></div></section>
  {data?.demo&&<div className="price-demo" role="status"><b>화면 예시</b> API 인증 전이라 예시 가격을 표시합니다. 실제 통계로 오해하지 마세요.</div>}
  <section className="price-kpis" aria-label="장바구니 주요 통계">
   ...
   <article><small>가격 기준일</small><b>{data?.asOfDate||'-'}</b><span>하루 한 번 갱신</span></article>
  </section>
  <section className="price-grid">
   <article className="price-panel"><header><div><small>ITEMS</small><h2>내 장바구니 구성</h2></div><span>수량을 0으로 하면 제외됩니다.</span></header><div className="price-items">{(data?.items||[]).map(item=>{const quantity=selected[item.itemCode]??item.quantity;const change=changeRate(item.currentPrice,item.monthAgoPrice);const signal=signalFor(item);return <div key={item.itemCode}><div><b>{item.name}</b><small>{item.unit} 기준</small></div><label><span className="sr-only">{item.name} 수량</span><select value={quantity} onChange={event=>setSelected(current=>({...current,[item.itemCode]:Number(event.target.value)}))}>{[0,1,2,3,4,5].map(value=><option key={value} value={value}>{value}개</option>)}</select></label><strong>{won.format(item.currentPrice)}</strong><span className={change!==null&&change>0?'up':'down'}>{rate(change)}</span><em className={`signal ${signal}`}>{signalLabel[signal]}</em></div>})}</div></article>
  ...
  <footer className="price-source"><b>데이터 출처</b><span>{data?.source} · 조사 평균가격이며 실제 매장 판매가격과 다를 수 있습니다.</span><a href="https://www.data.go.kr/data/15156063/openapi.do?recommendDataYn=Y" target="_blank" rel="noreferrer">공식 API 확인 →</a></footer>
 </main></div>
}
