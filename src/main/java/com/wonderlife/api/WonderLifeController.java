...
@RestController @RequestMapping("/api") public class WonderLifeController{
 ...
 @PostMapping("/histories") HistoryRow save(@AuthenticationPrincipal OAuth2User p,@Valid @RequestBody SaveRequest r){long uid=user(p);var key=new WonderLifeMapper.MutableId();mapper.insert(uid,r.calculatorType(),r.title(),r.inputJson(),r.resultJson(),key);return mapper.history(key.id,uid);}
 @Transactional @PostMapping("/histories/import") List<HistoryRow> importHistories(@AuthenticationPrincipal OAuth2User p,@Valid @RequestBody ImportRequest request){long uid=user(p);if(!request.histories().isEmpty())mapper.insertMany(uid,request.histories().stream().map(r->new WonderLifeMapper.ImportHistory(r.calculatorType(),r.title(),r.inputJson(),r.resultJson())).toList());return mapper.histories(uid);}
